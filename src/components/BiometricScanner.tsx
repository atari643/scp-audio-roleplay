    ...
    <div className="fixed inset-0 z-[9990] bg-black/90 flex items-center justify-center p-6 animate-fade-scale select-none">
      <div className="w-full max-w-sm bg-scp-surface border border-scp-border rounded-2xl overflow-hidden shadow-2xl">
        ...
        <div className="p-4 border-b border-scp-border flex items-center justify-between">
          <div>
            <div className="text-[10px] font-mono text-amber-400 tracking-widest font-semibold">
              SCiPNET // CONTRÔLE D'ACCÈS BIOMÉTRIQUE
            </div>
            <div className="text-sm font-mono text-white font-bold truncate mt-0.5">{scpTitle}</div>
          </div>
          <button
            onClick={handleInstantGrant}
            className="text-[10px] font-mono text-slate-500 hover:text-red-400 border border-slate-800 hover:border-red-800 px-2 py-1 rounded transition-colors"
          >
            [ PASSER ]
          </button>
        </div>
        ...
        <div className="p-6 flex flex-col items-center gap-5">
          <div
            className={`relative w-36 h-44 rounded-xl border-2 cursor-pointer select-none overflow-hidden transition-colors ${
              phase === 'holding' ? 'border-amber-500 shadow-lg shadow-amber-950/50' :
              phase === 'granted' ? 'border-emerald-500 shadow-lg shadow-emerald-950/50' :
              phase === 'denied'  ? 'border-red-500 animate-shake shadow-lg shadow-red-950/50' :
          ...
          >
            ...
            <svg viewBox="0 0 100 120" className="absolute inset-0 w-full h-full p-3 opacity-40">
              {[8,16,24,32,40,48,56,64,72,80].map((r, i) => (
                <ellipse
                  ...
                  rx={r * 0.65} ry={r * 0.55}
                  fill="none"
                  stroke={phase === 'granted' ? '#10b981' : phase === 'denied' ? '#ef4444' : '#06b6d4'}
                  strokeWidth="1.2"
                  opacity={0.4 + i * 0.06}
                />
              ))}
            ...
            {phase === 'granted' && (
              <div className="absolute inset-0 bg-emerald-950/80 flex items-center justify-center animate-fade-scale">
                <div className="text-center">
                  <div className="text-3xl mb-1 text-emerald-400 font-bold">✓</div>
                  <div className="text-emerald-400 font-mono font-bold text-xs">ACCÈS ACCORDÉ</div>
                </div>
              </div>
            )}
            {phase === 'denied' && (
              <div className="absolute inset-0 bg-red-950/80 flex items-center justify-center">
                <div className="text-center">
                  <div className="text-3xl mb-1 text-red-400 font-bold">✗</div>
                  <div className="text-red-400 font-mono font-bold text-xs">REFUSÉ</div>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="text-center w-full">
            <p className="text-xs font-mono text-slate-300 min-h-[32px] flex items-center justify-center text-center px-2 leading-tight">
              {getTelemetryText()}
            </p>

            <div className="w-full h-2 bg-slate-950 rounded-full overflow-hidden border border-slate-800 mt-2 p-0.5">
              <div
                className={`h-full rounded-full transition-none ${
                  phase === 'granted' ? 'bg-emerald-500' : 'bg-gradient-to-r from-red-600 to-amber-500'
                }`}
                style={{ width: `${holdProgress}%` }}
              />
            </div>
            <div className="text-[10px] font-mono text-slate-500 mt-1 flex justify-between px-1">
              <span>CANAL BIOMÉTRIQUE</span>
              <span className="text-slate-400 font-bold">{Math.round(holdProgress)}%</span>
            </div>
          </div>
        </div>
